...
const Accordion = ({ isInput, text }) => {
	return <span className={`${isInput ? "block" : "hidden"} w-full text-sm text-red-600 -mt-2`}>{text}</span>;
};
...
const FadeCarousel = ({ className = "", grayscale = false, images = [], imgIndex = 0 }) => {
	...
	return (
		<CrossfadeCarousel
			className={`grayscale hover:grayscale-0 transition-all cursor-pointer rounded ${grayscale && "grayscale-0"} ${isImages && "skeleton"} ${className}`}
			interval={1500}
			transition={1500}
...
const LoginForm = (props) => {
	const { input, setInput, isInput, setIsInput, handleInput, setGrayscale, loading, setLoading, restartForm } = props;
	...
	const handleLogin = async (e) => {
		...
		if (!status) return res.message.includes("User") ? setIsInput({ ...isInput, email: true }) : setIsInput({ ...isInput, password: true });
	...
	return (
		<>
			<h1 className="text-4xl font-bold w-full my-10 text-center">Selamat Datang</h1>
			<Form
			...
			>
				...
				<span className="w-full text-sm flex justify-end">
					<Link
						to="/ganti-akun"
						className="w-fit h-fit text-blue-400"
					>
						Lupa kata sandi?
					</Link>
				</span>
				<Button type="submit">{loading ? <span className="loading loading-spinner loading-xs"></span> : "Masuk"}</Button>
...
const RegisterForm = (props) => {
	const { input, setInput, handleInput, isInput, setIsInput, setGrayscale, loading, setLoading, restartForm } = props;
	const handleRegister = async (e) => {
		e.preventDefault();
		if (input.password !== input.confirmPassword) return setIsInput({ ...isInput, confirmPassword: true });
		setLoading(true);
	...
	return (
		<>
			...
			>
				...
				/>
				<Button type="submit">{loading ? <span className="loading loading-spinner loading-xs"></span> : "Daftar"}</Button>
